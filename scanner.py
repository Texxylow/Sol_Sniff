...
async def fetch_new_pools(client: httpx.AsyncClient) -> list:
    try:
        r = await client.get(GECKO_URL, params={"page": 1}, headers=HEADERS, timeout=TIMEOUT)
        r.raise_for_status()
...
def compute_score(pool_attrs: dict, security: dict) -> dict:
    ...
    owner_ok    = security.get("owner_address") in ("", None, "0x0000000000000000000000000000000000000000")
    ...
    penalty_flags = []
    if is_mintable:
        penalties += 30; penalty_flags.append("Mintable")
    if freeze_auth:
        penalties += 20; penalty_flags.append("Freeze Authority")
    if not owner_ok and security:
        penalties += 20; penalty_flags.append("Ownership Not Renounced")
    if top10_pct >= 80:
        penalties += 20; penalty_flags.append("High Holder Concentration")
